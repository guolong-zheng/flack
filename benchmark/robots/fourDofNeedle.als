...
one sig fourDOF_needle extends ArmType{}{
	inverseKSolver = IKSolver
}

one sig fourDOF_needleArm extends RobotArm{}{
	armModel = fourDOF_needle
}
...
check repair_assert_1 for 4 but 8 Plugin

pred repair_pred_1 {
all a: RobotControl.output | a not in ArmType.anglelimit
implies ProduceFeedback[a]
}
run repair_pred_1 for 4 but 8 Plugin